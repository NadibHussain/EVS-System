...
const executiveVoteUrl = url.executiveListUrl;

class ResultExecutive extends Component {
  ...
  render() {
    ...
    return (  
<div > 
                <h3 style={{textAlign:'center'}}> Election Live Result ( Executive )</h3>
                <ul style={{listStyle:'none',alignItems:'center',justifyContent:'center',flexWrap:'wrap',flexDirection:'row',display:'flex'}}>
                    <PoseGroup>{result.map(id => <Item key={id.membershipId} > 
                      <Card
                        hoverable
                        style={{ width: 200, height: 350, padding: 0, margin:10 }}
...
export default ResultExecutive;