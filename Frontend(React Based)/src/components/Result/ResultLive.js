    ...
    this.interval = setInterval(() => axios.all([this.getPresidentResult(),this.getExecutiveResult(),this.getVoterList()])
      .then(axios.spread((presidentData, executiveData, voterList)=> {
        ...
        // Both requests are now complete
          const totalVoteGiven = president.reduce((a, b) => a + b.count, 0);
          const voteRemaining = (totalVoter - totalVoteGiven)
          
          // with 2 digit float number
        ...
        return ( 
            <div style={{backgroundColor:'#e6f7ff'}}> 
                ...
                <Row>
                  <Col xs = {20}>
                    <div> 
                        <h4 style={{marginLeft:220,textAlign:'center', textTransform:'uppercase',color:'blue', paddingTop:'10px'}}> Live President Result </h4>
                            ...
                            <PoseGroup>{resultPresident.map(id => <Item key={id.membershipId} > 
                              ...
                              <Meta
                                ...
                                //   description = {<div><h6 className=" text-success">{id.name}</h6> <p className="font-weight-bold" >{"Id: " + id.membershipId}</p></div>} 
                                  title =  {<Row >
                                                <Col xs={14} ><h6 className="font-weight-bold">{id.name}</h6><p>{"ID: " + id.membershipId}</p></Col>
                                                <Col xs = {10} className="text-center" ><div style={{ marginTop: 0, padding:0, color:'red', fontSize:30, alignContent:'center'}}>{id.count}</div></Col>
                                            </Row> 
                                            } 
                    ...
                    {/* president result end */}
                    {/* Executive result part */}
    
                    <div> 
                    <h4 style={{textAlign:'center',textTransform:'uppercase',color:'blue'}}> Live Executive Result </h4>
                          ...
                          >
                            ...
                            //   description = {<div><h6 className=" text-success">{id.name}</h6> <p className="font-weight-bold" >{"Id: " + id.membershipId}</p></div>} 
                              title =  {<Row>
                                            ...
                                            <Col span = {10} className="text-center" ><div style={{ marginTop: 0, padding:0, color:'red', fontSize:30, alignContent:'center'}}>{id.count}</div></Col>
                                        </Row> } 
